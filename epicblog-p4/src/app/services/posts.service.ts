...
export class PostsService {
  ...
  get featuredPost(): iPost {
    const featuredIndex: number = Math.floor(Math.random() * 30);
    return this.posts[featuredIndex];
  }
  ...
  getFilteredPost(tag: string): iPost[] {
    return (this.filteredPosts = this.posts.filter((post: iPost) =>
